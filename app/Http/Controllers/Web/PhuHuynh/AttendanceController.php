...
class AttendanceController extends Controller
{
    ...
    public function absence_history($id)
    {
        $history = Kid::where('id', $id)->with(['attendance' => function ($query) {
            $query->whereBetween("date", [substr(Carbon::now(), 0, 7) . '-1', substr(Carbon::now(), 0, 10)])->whereIn('status', ['0', '2'])->where('arrival_time', "00:00:00")->orderBy('date', 'desc');
        }])->first();
        $future = Kid::where('id', $id)->with(['attendance' => function ($query) {
            $query->whereBetween("date", [substr(Carbon::now()->add('1', 'day'), 0, 10), substr(Carbon::now(), 0, 7) . '-31'])->whereIn('status', ['0', '2'])->where('arrival_time', "00:00:00")->orderBy('date', 'asc');
        }])->first();
